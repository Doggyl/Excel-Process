...
filepath = ('/Users/liujunyuan/Desktop/CLD200701.xls')

data = xlrd.open_workbook(filepath)
sheet = data.sheet_by_name("T")
sheet_RH = data.sheet_by_name("RH")
sheet_W10A = data.sheet_by_name("W10A")
sheet_R = data.sheet_by_name("R")
...
month = int(sheet.cell(3,10).value)
year = int(sheet.cell(3,7).value)


workbook = xlwt.Workbook()
worksheet = workbook.add_sheet("year"+str(year),cell_overwrite_ok= True)
...
for j in range(0,31,1):
    ...
    for i in sequence:
        ...
        day = sheet.cell(j+5,0).value
        par_T = sheet.cell(j+5,i).value
        par_W10A = sheet_W10A.cell(6+2*j,i+1).value
        par_RH = sheet_RH .cell(j+5,i).value
        par_R = sheet_R .cell(j+5,i).value
        par_Eg = data.sheet_by_name(D_sheet_name[j]).cell(sequence.index(i)+5,1).value
        par_PAR = data.sheet_by_name(D_sheet_name[j]).cell(sequence.index(i)+5,5).value

        row = sequence.index(i)+1
        worksheet.write(row+24*j,0,day)
        worksheet.write(row+24*j,1,year)
...
workbook.save('/Users/liujunyuan/Desktop/2007_test.xls')
